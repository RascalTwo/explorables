...
`mount(host)` is mandatory — **289/289 existing variants have one. There is no
...
All 109 modules do this; 145 debugger variants total. It is the most expensive
...
1. **Best — the user supplies their own input.** A text field, a number field,
   a slider, a clickable grid. 103/109 modules do this — and "their own input"
...
#### Which challenge is live right now — the weekly back-fill question

The replay maps a calendar date onto the **same month and day of the original
run**, so today's challenge is the one first published on that month-day in
2025/2026. `/daily-coding-challenge/today` returns it directly, and its `date`
field is the **original** date, not today's — which is exactly the date to put in
`dates`. Two probes settle "what am I missing?":

```
curl -s https://api.freecodecamp.org/daily-coding-challenge/today
```

Read `challengeNumber` off that, compare against the highest `n` already in
`challenges/`, and the gap is the back-fill. Worked example — on 2026-09-19
`/today` returned `challengeNumber: 40` with `"date":"2025-09-19"`, while the
gallery stopped at #26; the 14 missing days were #27–#40, dated 2025-09-06 through
2025-09-19. Do **not** derive the gap by counting days since the replay began: a
written-down `challengeNumber` survives a skipped day and arithmetic does not,
which is the same rule [Dates](#dates) applies to the date itself.
...
- **#1–292 is the only territory left**, and 40 of it is done (#1–40). All of it
  is reachable through the same `/date/` endpoint, since those dates are past.

That is a fixed, finite back-fill of 252 challenges rather than a backlog that
...
- **`cost`** — a short string in the demo header describing the work done, e.g.
  ...
  Step-throughs use `"line-by-line"` (54 of 145) — or, better, a phrase naming
...
- **`code`** — the real, copy-pasteable TypeScript solution as a template
  ...
  non-debugger variant (144/144); **absent from every step-through variant**
  (145/145), because the debugger renders its own source. That's a rule, not an
...
(107/109 modules) with a 2–3 character prefix:
...
Ask before you build. **Most don't, and that's the normal case** — 74 of 109
...
**Two approaches is the goal *where the problem supports it*** — 34 of 109
modules do it:
...
count — it reports 33 where the true number is 34. The count was left keyed to the
tint on purpose. The obvious structural alternative, "two or more `Step:` variants",
reports 35, because it sweeps in `318`'s `graded: false` detour, which is not a
