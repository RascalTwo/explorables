...
`mount(host)` is mandatory — **217/217 existing variants have one. There is no
...
All 81 modules do this; 109 debugger variants total. It is the most expensive
...
1. **Best — the user supplies their own input.** A text field, a number field,
   a slider, a clickable grid. 75/81 modules do this — and "their own input"
...
- **#1–292 is the only territory left**, and 12 of it is done (#1–12). All of it
  is reachable through the same `/date/` endpoint, since those dates are past.

That is a fixed, finite back-fill of 280 challenges rather than a backlog that
...
- **`cost`** — a short string in the demo header describing the work done, e.g.
  ...
  Step-throughs use `"line-by-line"` (53 of 109) — or, better, a phrase naming
...
- **`code`** — the real, copy-pasteable TypeScript solution as a template
  ...
  non-debugger variant (108/108); **absent from every step-through variant**
  (109/109), because the debugger renders its own source. That's a rule, not an
...
(79/81 modules) with a 2–3 character prefix:
...
Ask before you build. **Most don't, and that's the normal case** — 54 of 81
...
**Two approaches is the goal *where the problem supports it*** — 26 of 81
modules do it:
