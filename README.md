...
The visualizations are built with my custom **`/viz` skill**, which now lives in
[RascalTwo/ai-setup](https://github.com/RascalTwo/ai-setup/tree/main/skills/viz) — a
spec-compliant [Agent Skill](https://agentskills.io) that serves ad-hoc HTML/CSS/JS
visualizations at a live, hot-reloading local URL, keeps every page in git, and can publish
any of them as a single self-contained HTML file to a static host.

**Want it in your own agent?** `npx skills add RascalTwo/ai-setup -s viz` drops `/viz` into
whatever AI coding agent you use (Claude Code, Codex, Copilot, Cursor, Gemini…); update with
`npx skills update`. Full guide: [ai-setup `skills/viz/`](https://github.com/RascalTwo/ai-setup/tree/main/skills/viz).

## Layout

- **`viz-pages/`** — the explorables themselves. Each is a self-contained folder with an
  `index.html`; open it in a browser, no build step. Built with the
  [`/viz` skill](https://github.com/RascalTwo/ai-setup/tree/main/skills/viz).
